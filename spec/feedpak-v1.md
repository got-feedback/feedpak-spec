...
2. **Legacy opaque** — `definitions`, a list of raw tone objects copied verbatim from the source
   (e.g. a psarc `GearList`). It is source-specific and not portably interpretable; Readers that do
...
passthrough. It is deliberately **not modelled on any one source or engine** (a psarc `GearList`, a
single serial chain, a fixed four-knob gear catalog, or one plugin format): a rig is an ordered set
